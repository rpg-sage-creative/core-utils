...
function parseKeyValuePair<T extends string = string>(input: string, index: number): OrUndefined<TKeyValueIndex<T>> {
...
export class ArgsManager<T extends string> extends Collection<T> {
	...
	public keyValuePairs<U extends string = string, V extends TKeyValuePair<U> = TKeyValuePair<U>>(): V[] {
		const pairs = this.parseKeyValuePairs<U>() as OrUndefined<V>[];
